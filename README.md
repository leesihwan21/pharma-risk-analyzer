...
![Python](https://img.shields.io/badge/Python-3.11-blue) ![Flask](https://img.shields.io/badge/Flask-3.1-lightgrey) ![XGBoost](https://img.shields.io/badge/XGBoost-ML-orange) ![FAERS](https://img.shields.io/badge/FDA-FAERS-red) ![AWS](https://img.shields.io/badge/AWS-EC2%2FRDS-yellow)
...
```mermaid
graph TD
    A["🖥️ User Browser (PWA)"] --> B

    subgraph APP["Flask Application — gunicorn · systemd"]
        B["Routes\nanalysis · drug · ae · auth"]
        C["RAG\nBM25 + FAISS · RRF 융합"]
        D["ML Dashboard\nMLflow · Vision"]
        E["AE Manager\n21 CFR · ICH E2B(R3)"]
    end
    ...
    subgraph ML["ML / MLOps"]
        F["XGBoost · Optuna · SHAP/LIME\nMLflow · Prophet · Calibration\nDrift Monitoring · YOLOv8 · OCR"]
    end

    subgraph AI["AI / External APIs"]
        G["Anthropic Claude (Prompt Caching)\nOpenFDA · PubMed · MFDS DUR\n식약처 OpenAPI"]
    end
    ...
    F --> DB1["🗄️ AWS RDS PostgreSQL\n(VPC-only, SSH tunnel)"]
    G --> DB2["📁 SQLite\nMLflow · pill_identity.db"]
    H --> DB3["💾 File Storage\nmodel.pkl · FAISS index"]

    DB1 & DB2 & DB3 --> INFRA

    subgraph INFRA["☁️ AWS Infrastructure"]
        I["EC2 t3.micro · Ubuntu 24.04 · 서울 리전\nElastic IP · EBS 30GB · Swap 2GB"]
        J["Security\nIAM · Fail2ban · SSH restrict · SG"]
        K["CI/CD & Monitoring\nGitHub Actions · pytest 117개 · CloudWatch · Docker"]
    end
```
...
| 기능 | 설명 |
|------|------|
| RAG 약물 Q&A | BM25 + FAISS 하이브리드 검색(RRF 융합) → Anthropic Claude 답변 생성 + [출처: doc_id] 인라인 citation 표기 + Prompt Caching 적용(반복 질의 시 토큰 90% 절감) |
...
FAERS 데이터는 동일 환자(primaryid)가 여러 약물·여러 부작용으로 중복 행에 등장할 수 있습니다. 단순 train_test_split을 사용하면 같은 환자의 다른 레코드가 train과 test에 동시에 들어가 평가 지표가 실제보다 부풀려질 수 있습니다.
...
추가로, `drug_risk_rate` / `reac_risk_rate` / `combo_risk_rate` 같은 위험률 피처를 train 데이터로만 계산한 뒤 test에 매핑하여(train에 없는 값은 global rate로 fallback) 피처 자체의 정보 누수도 차단했습니다. 코드 내에는 train/test 환자 ID 중복 여부를 assert로 검증하는 안전장치가 있어 누수가 없음을 보장합니다.
...
**Target Encoding 개선 효과:** 원래 피처셋에는 drug_encoded/reac_encoded(의미 없는 LabelEncoder 정수 ID)가 risk-rate 피처와 함께 그대로 모델 입력으로 들어가 있었습니다. 이 원시 ID를 제거하고 risk-rate 피처에 표본 수 기반 Bayesian 스무딩(`(count·mean + k·global_rate) / (count + k)`, k=20)을 적용한 결과, LogisticRegression의 F1이 0.351 → 0.406으로 크게 개선되었습니다.
...
- **SHAP 설명 가능성과의 통합:** XGBoost는 SHAP TreeExplainer와 호환성이 가장 안정적이며, 이미 Global/Local 설명 파이프라인이 구축되어 있습니다.
- **안정적인 Optuna 최적화 워크플로우:** 9개 하이퍼파라미터에 대한 베이지안 탐색 파이프라인이 XGBoost 기준으로 구축되어 있어, 추가 데이터 확보 시 즉시 재학습이 가능합니다.
- **빠른 재학습 파이프라인:** Drift Monitoring 결과가 보여주듯 분기별 재학습이 필수적인데, XGBoost는 RandomForest보다 학습 속도가 빠르고 증분 학습 확장에 유리합니다.
- **프로덕션 배포 확장성:** 모델 직렬화 크기, 추론 속도, gunicorn 환경에서의 메모리 사용량 면에서 XGBoost가 더 가볍습니다.

즉 "어떤 모델이 가장 높은 점수를 받았는가"가 아니라 "어떤 모델이 SHAP·Optuna·재학습 자동화까지 포함한 전체 MLOps 파이프라인에 가장 잘 맞는가"를 기준으로 선택했습니다.
...
**평가 누수 버그 발견 및 수정:** 처음 측정했을 때는 학습 분기(2024Q1)의 F1이 0.752로 다른 분기(0.38 전후)보다 훨씬 높게 나왔습니다. 학습 분기를 평가할 때 학습에 실제로 쓰인 80% 데이터까지 포함된 전체 Q1으로 평가하고 있었다는 버그를 발견하여, held-out 20%로만 평가하도록 수정했습니다.
...
수정 후에는 모든 분기가 F1 0.38~0.39 사이에 머물러 실질적인 드리프트가 거의 관찰되지 않습니다. 처음 발견했던 "심각한 드리프트"는 대부분 모델의 시간적 불안정성이 아니라 평가 방식 자체의 버그였다는 결론입니다.

---

## 📚 RAG 평가 | RAG Evaluation

PubMed 기반 RAG 챗봇의 응답 품질을 정량적으로 평가했습니다. RAGAS 라이브러리는 의존성 충돌로 직접 실행이 불가능해, 동일한 평가 방법(Faithfulness, Answer Relevancy, Context Precision)을 LLM judge(llama3.1:8b) 호출로 직접 구현했습니다. 자기평가 영향을 줄이기 위해 답변 생성(llama3.2)과 평가(llama3.1:8b) 모델을 분리했습니다.
...
**진단:** Context Precision이 상대적으로 낮아, 생성(Generation) 단계보다 검색(Retrieval) 단계가 RAG 파이프라인의 병목임을 시사합니다. 이후 임베딩 모델 교체(의학 전문 특화 임베딩) 또는 청크 분할 전략 개선이 우선 과제로 식별되었습니다.
...
검증       : GroupShuffleSplit(데이터 누수 방지), Smoothed Target Encoding(과적합 방지),
             Calibration(Isotonic), Drift Monitoring(평가 누수 버그 수정),
             RAG 평가(Faithfulness/Relevancy/Context Precision)
Vision     : YOLOv8 (알약 탐지), EasyOCR (식별문자 인식)
추천 시스템 : K-Means 클러스터링, Co-medication 연관 분석
RAG        : LangChain, FAISS + BM25 하이브리드 검색(RRF), sentence-transformers,
...
Compliance : 21 CFR Part 11 일부 요구사항 구현(전자서명, Audit Trail — 공식 인증 아님),
             ICH E2B(R3) XML 포맷 구현
Frontend   : Jinja2 Templates, Vanilla JS, 반응형 CSS, PWA
Container  : Docker (빌드·실행 검증 완료, torch CPU 전용 빌드로 경량화)
Infra      : AWS EC2 (t3.micro, Ubuntu 24.04, 서울 리전) + AWS RDS PostgreSQL +
             Elastic IP + gunicorn + systemd + IAM + CloudWatch
CI/CD      : GitHub Actions (push/PR 시 pytest 자동 실행)
Test       : pytest (117개 자동 테스트)
```
...
- **배포 방식:** gunicorn(3 workers) + systemd 서비스(pharma.service)로 24시간 운영 및 장애 시 자동 재시작. EnvironmentFile로 .env 환경변수 안전하게 로드
...
- **IAM 역할 기반 권한 관리:** CloudWatchAgentServerPolicy + AmazonS3FullAccess를 포함한 전용 IAM 역할(pharma-risk-analyzer-ec2-role)을 EC2 인스턴스에 연결
- **모니터링:** CloudWatch 에이전트로 gunicorn 애플리케이션 로그 및 CPU·메모리·디스크 메트릭(CWAgent 네임스페이스)을 실시간 수집
...
| + Target Encoding 개선 (최종) | 51.9% | 0.412 | 0.473 |
...
초기에는 Accuracy 69%라는 결과에 만족할 수도 있었지만, FAERS 데이터의 구조(동일 환자가 여러 레코드에 등장)를 고려하면 이 수치를 그대로 신뢰할 수 없다는 점을 인식하고 데이터 누수 검증을 진행했습니다. 그 결과 실제 성능은 52%로 재산정되었고, 이는 이후 Drift Monitoring 실험으로 이어지는 계기가 되었습니다. 이 과정 자체가 모델 성능 수치보다 더 중요한 배움이라고 생각하여 README에도 정직하게 기록했습니다.
...
The resulting performance decreased to 52%, but became significantly more reliable and realistic. This experience highlighted the importance of **data validation over raw model metrics** — a lower, honest number is more valuable than a higher, leaked one. The same validation mindset was extended further: Calibration testing revealed that raw model probabilities did not match real-world outcome rates (improved via isotonic regression), and Drift Monitoring uncovered an evaluation pipeline bug that was masking stable cross-quarter performance.
