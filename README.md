...
- AWS EC2 gunicorn + systemd 운영, IAM 역할 기반 권한 관리, CloudWatch 로그·메트릭 모니터링
...
DB         : SQLite (mlflow.db, pill_identity.db 포함)
...
Infra      : AWS EC2 (t3.micro, Ubuntu 24.04, 서울 리전) + Elastic IP + gunicorn + systemd + IAM + CloudWatch
...
본 도구는 **연구·교육·포트폴리오 목적**으로 제작되었으며, 실제 임상 처방 결정에 사용해서는 안 됩니다.
