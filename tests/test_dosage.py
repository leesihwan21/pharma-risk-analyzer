...
class TestCrCl:
    ...
    def test_male_normal_renal_function(self, client):
        res = post(
            client,
            "/api/dosage/crcl",
            {"age": 40, "weight": 70, "creatinine": 1.0, "sex": "M"},
        )
    ...
    def test_female_applies_correction_factor(self, client):
        res_m = post(
            client,
            "/api/dosage/crcl",
            {"age": 50, "weight": 60, "creatinine": 1.0, "sex": "M"},
        )
        res_f = post(
            client,
            "/api/dosage/crcl",
            {"age": 50, "weight": 60, "creatinine": 1.0, "sex": "F"},
        )
    ...
    def test_severe_renal_impairment_stage(self, client):
        ...
        res = post(
            client,
            "/api/dosage/crcl",
            {"age": 85, "weight": 40, "creatinine": 8.0, "sex": "F"},
        )
        data = json.loads(res.data)
        assert data["crcl"] < 15
    ...
    def test_zero_age_returns_400(self, client):
        res = post(
            client, "/api/dosage/crcl", {"age": 0, "weight": 70, "creatinine": 1.0}
        )
        assert res.status_code == 400
...
class TestPediatricDosage:
    ...
    def test_bsa_rule_requires_weight_and_height(self, client):
        res = post(
            client,
            "/api/dosage/pediatric",
            {"adult_dose": 100, "weight": 20, "height": 110},
        )
        data = json.loads(res.data)
...
class TestBSA:
    ...
    def test_total_dose_with_dose_per_m2(self, client):
        res = post(
            client, "/api/dosage/bsa", {"weight": 70, "height": 170, "dose_per_m2": 100}
        )
        data = json.loads(res.data)
