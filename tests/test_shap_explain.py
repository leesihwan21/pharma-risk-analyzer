...
def post(client, payload):
    return client.post(
        "/api/shap/explain", data=json.dumps(payload), content_type="application/json"
    )
...
class TestShapExplain:
    def test_unknown_drug_returns_400(self, client):
        res = post(
            client,
            {"drug": "UNKNOWN_XYZ", "reaction": "FATIGUE", "age": 50, "sex": "F"},
        )
    ...
    def test_unknown_reaction_returns_400(self, client):
        res = post(
            client,
            {"drug": "METHOTREXATE", "reaction": "UNKNOWN_XYZ", "age": 50, "sex": "F"},
        )
        assert res.status_code == 400
    ...
    def test_valid_input_returns_top_features_and_explanation(self, client):
        ...
        """
        with patch(
            "app.routes.analysis.shap_xai.compute_shap", return_value=MOCK_SHAP_RESULT
        ):
            res = post(
                client,
                {"drug": "METHOTREXATE", "reaction": "FATIGUE", "age": 50, "sex": "F"},
            )
