...
def detect_pill():
    ...
    target_drug = (
        drug_hint
        if drug_hint
        else (detected_drugs[0] if len(detected_drugs) > 0 else None)
    )
    ...
    if target_drug:
        try:
            ...
            risk_rates = pickle.load(
                open(os.path.join(MODEL_DIR, "risk_rates.pkl"), "rb")
            )
    ...
    if len(detected_drugs) >= 2:
        try:
            ...
            for drug in detected_drugs[:2]:
                ...
                df = load_df()
                top_reacs = (
                    df[df["drugname"].str.upper() == drug]["pt"]
                    .value_counts()
                    .head(3)
                    .index.tolist()
                )
...
def search_drug(drugname):
    ...
    try:
        log = DrugSearch(
            drugname=drugname.upper(), total_reports=len(result), age_avg=age_avg
        )
        db.session.add(log)
        db.session.commit()
    except:
...
def drug_detail(drugname):
    ...
    return render_template(
        "drug_detail.html", drugname=drugname, stats=stats, charts=charts
    )
...
def generate_report(drugname):
    ...
    # ????
    story.append(Paragraph("Pharma Risk Analyzer - Drug Report", title_style))
    story.append(Paragraph(f"Drug: {drugname}", sub_style))
    story.append(
        Paragraph(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M')}", sub_style)
    )
    ...
    stats_data = [
        ...
        [
            "Average Age",
            f"{round(float(age_data.mean()), 1) if len(age_data) > 0 else 'N/A'}",
        ],
        [
            "Age Range",
            (
                f"{int(age_data.min())} ~ {int(age_data.max())}"
                if len(age_data) > 0
                else "N/A"
            ),
        ],
        [
            "Female (%)",
            f"{round(len(result[result['sex']=='F']) / len(result) * 100, 1)}%",
        ],
        [
            "Male (%)",
            f"{round(len(result[result['sex']=='M']) / len(result) * 100, 1)}%",
        ],
    ]
...
def get_history():
    ...
    predictions = (
        PredictionLog.query.order_by(PredictionLog.predicted_at.desc()).limit(20).all()
    )
    return jsonify(
        {
...
def drug_network(drugname):
    ...
    for reac, cnt in top_reac.items():
        nodes.append(
            {"id": reac, "type": "reaction", "size": max(8, min(20, cnt // 10))}
        )
...
def register():
    if request.method == "POST":
        ...
        login_user(user)
        return jsonify(
            {"message": "회원가입 성공!", "role": user.role, "username": username}
        )
...
def compare_drugs():
    ...
    def get_stats(result, name):
        ...
        return {
            ...
            "age_avg": round(float(age_data.mean()), 1) if len(age_data) > 0 else 0,
            "female_pct": round(
                len(result[result["sex"] == "F"]) / len(result) * 100, 1
            ),
    ...
    return jsonify(
        {"drug1": get_stats(result1, drug1), "drug2": get_stats(result2, drug2)}
    )
...
def korea_dashboard():
    ...
    for symptom in top5:
        ...
        for y in years:
            ...
            counts.append(
                int(row[col].values[0]) if len(row) > 0 and col in df.columns else 0
            )
...
def ae_list():
    status_filter = request.args.get(
        "status", ""
    )  # overdue / urgent / warning / normal / submitted
...
def ae_create():
    ...
    # ????? u?
    if (
        not data.get("patient_code")
        or not data.get("drugname")
        or not data.get("ae_term")
    ):
            ...
            jsonify(
                {
                    ...
                    "ctcae_grade": ctcae_grade,
                    "report_deadline": (
                        report_deadline.strftime("%Y-%m-%d")
                        if report_deadline
                        else None
                    ),
                }
...
def ae_pdf(ae_id):
    ...
    patient_data = [
        ...
        [
            "Sex",
            "Female" if report.sex == "F" else "Male" if report.sex == "M" else "N/A",
        ],
    ]
    ...
    ae_data = [
        ...
        [
            "Onset Date",
            (
                report.ae_start_date.strftime("%Y-%m-%d")
                if report.ae_start_date
                else "N/A"
            ),
        ],
        [
            "End Date",
            (
                report.ae_end_date.strftime("%Y-%m-%d")
                if report.ae_end_date
                else "Ongoing"
            ),
        ],
    ...
    report_data = [
        ...
        [
            "Deadline",
            (
                report.report_deadline.strftime("%Y-%m-%d")
                if report.report_deadline
                else "N/A"
            ),
        ],
...
def ae_e2b(ae_id):
    ...
    sae_flags = {
        ...
        "seriousnesscongenitalanomali": (
            "1" if report.sae_category == "??o?????" else "0"
        ),
    ...
    # ??????? ??? (1=????, 2=????)
    causality_code = (
        "1" if report.causality in ["Certain", "Probable", "Possible"] else "2"
    )
