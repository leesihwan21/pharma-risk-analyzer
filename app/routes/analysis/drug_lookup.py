...
def api_drug_lookup():
    ...
    try:
        ...
        if items:
            item = items[0]
            results['korean'] = {
                'name': item.get('ITEM_NAME', ''), 'company': item.get('ENTP_NAME', ''),
                'shape': item.get('DRUG_SHAPE', ''), 'color': item.get('COLOR_CLASS1', ''),
                'etc_otc': item.get('ETC_OTC_CODE', ''), 'class_name': item.get('CLASS_NAME', ''),
    ...
    try:
        r    = http_requests.get('https://api.fda.gov/drug/label.json',
               params={'search': f'openfda.brand_name:"{drugname.upper()}" OR openfda.generic_name:"{drugname.upper()}"',
                       'limit': 1}, timeout=5)
...
def api_drug_shape():
    ...
    try:
        ...
        result = [{
            ...
            'shape': item.get('DRUG_SHAPE', ''), 'color': item.get('COLOR_CLASS1', ''),
            'etc_otc': item.get('ETC_OTC_CODE', ''), 'class_name': item.get('CLASS_NAME', ''),
            'chart': item.get('CHART', ''), 'img_url': item.get('ITEM_IMAGE', ''),
            'print_front': item.get('PRINT_FRONT', ''), 'print_back': item.get('PRINT_BACK', '')
        } for item in items]
    ...
    try:
        payload = {
            ...
            'messages': [{'role': 'user', 'content': [
                ...
                {'type': 'text',  'text': '이 이미지에서 약물/의약품을 식별하여 약물명만 간단하게 답해주세요.'}
            ]}]
        }
        ...
        result = r.json()
        if 'usage' in result:
            print(f"[Anthropic Usage] input={result['usage'].get('input_tokens')} "
...
def soc_analysis(drugname: str):
    ...
    for reac, cnt in top_reac.items():
        ...
        mapped_reactions.append({
            'pt': reac, 'count': int(cnt),
            'pct': round(cnt / len(result) * 100, 2), 'soc': soc
        })
    ...
    return jsonify({
        'drugname': drugname, 'total_reports': len(result),
        'soc_summary': soc_summary, 'reactions': mapped_reactions,
