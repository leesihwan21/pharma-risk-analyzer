...
def api_trend():
    ...
    trend = (filtered.groupby('quarter').size()
...
def api_trend_forecast():
    ...
    try:
        ...
        for r in result_rows:
            r['actual'] = int(actual_map[r['quarter']]) if r['quarter'] in actual_map else None

        return jsonify({
            'drug': drugname, 'forecast': result_rows,
            'quarters': len(trend), 'forecast_periods': 2
        })
