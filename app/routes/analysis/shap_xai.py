...
def compute_shap(drugname: str, reaction: str, age: float, sex: str) -> dict:
    ...
    risk_rates = pickle.load(open(
        os.path.join(current_app.config['MODEL_DIR'], 'risk_rates.pkl'), 'rb'
    ))
...
def api_lime():
    ...
    try:
        ...
        exp  = explainer.explain_instance(
            ...
            num_features=7, num_samples=300, labels=(1,)
        )
        pred = int(model.predict(x)[0])
        prob = model.predict_proba(x)[0]
        ...
        for feat_expr, weight in exp.as_list(label=1):
            ...
            lime_result.append({'feature': matched, 'expression': feat_expr,
        ...
        return jsonify({
            ...
            'prediction': pred,
            'risk_label': 'HIGH RISK' if pred == 1 else 'LOW RISK',
            'probability': {'safe': round(float(prob[0])*100, 1),
                            'risk': round(float(prob[1])*100, 1)},
            'lime': lime_result,
            'feature_values': {
                'drug': drugname, 'reaction': reaction, 'sex': sex, 'age': age,
