...
def read_imprint(img_np):
    ...
    for k in range(4):
        ...
        combined_text = re.sub(
            r"[^A-Z0-9]", "", "".join([r[1] for r in results]).upper()
        )
        avg_conf = sum(r[2] for r in results) / len(results)
        print(
            f"[OCR DEBUG] 회전 {k * 90}도 → 텍스트: '{combined_text}', 신뢰도: {avg_conf:.2f}"
        )
    ...
    if best_conf < MIN_OCR_CONFIDENCE:
        print(
            f"[OCR DEBUG] 최종 신뢰도({best_conf:.2f})가 기준치 미달 — 인식 실패로 처리"
        )
        return ""
...
def lookup_pill_by_imprint(imprint_text):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT * FROM pill_identity WHERE print_front = ? LIMIT 1", (imprint_text,)
    )
    row = cur.fetchone()
    if not row:
        cur.execute(
            "SELECT * FROM pill_identity WHERE print_front LIKE ? LIMIT 1",
            (f"%{imprint_text}%",),
        )
        row = cur.fetchone()
    conn.close()
...
def detect_pill():
    ...
    dur_check = None
    target_drug = (
        drug_hint if drug_hint else (detected_drugs[0] if detected_drugs else None)
    )
        ...
        try:
            ...
            if combo_temp_results:
                ...
                if api_key and len(detected_drugs) >= 2:
                    dur_check = check_combo_dur_taboo(
                        detected_drugs[0], detected_drugs[1], api_key
                    )
                else:
                    dur_check = {
                        "checked": False,
                        "is_taboo": False,
                        "reason": "API 키 없음 또는 약물 부족",
                    }
        except Exception as e:
...
def detect_and_lookup():
    ...
    return jsonify(
        {"detections": detections, "image": img_b64, "mfds_results": mfds_results}
    )
