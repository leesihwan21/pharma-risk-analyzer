...
def api_polypharmacy():
    ...
    if all_ids:
        ...
        serious_all = df_all[df_all["outc_cod"].isin(serious_outcomes)][
            "primaryid"
        ].nunique()
    ...
    overall_risk = (
        round(sum(p["risk_score"] for p in pairs) / len(pairs), 1) if pairs else 0
    )
...
def api_drug_vision():
    ...
    try:
        ...
        if "usage" in result:
            print("==================================================")
            print("ℹ️ [Anthropic Usage Log]")
            print(f" - Input Tokens: {result['usage'].get('input_tokens')}")
            print(f" - Output Tokens: {result['usage'].get('output_tokens')}")
            print(
                f" - Cached Creation Tokens: {result['usage'].get('cache_creation_input_tokens', 0)}"
            )
            print(
                f" - Cached Read Tokens: {result['usage'].get('cache_read_input_tokens', 0)}"
            )
            print("==================================================")
...
def calculate_ebgm(drugname):
    ...
    for reac in top_reactions:
        ...
        c = len(
            df[(df["drugname"].str.upper() != drugname) & (df["pt"] == reac)]
        )  # 타약물+반응
...
def api_trend_forecast():
    ...
    try:
        ...
        # quarter -> datetime 변환 (Prophet은 ds 컬럼 필요)
        ...
        for reac in list(all_reactions)[:8]:
            ...
            for r in results:
                ...
                trend_by_reaction[reac].append(
                    {"quarter": r["quarter"], "prr": prr_map.get(reac, 0)}
                )
