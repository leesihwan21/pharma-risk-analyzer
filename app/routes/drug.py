...
def predict():
    ...
    try:
        log = PredictionLog(
            drugname=drugname, reaction=reaction,
...
평균 나이: {round(float(age_data.mean()), 1) if len(age_data) > 0 else 'N/A'}세"""
        ...
        response   = http_requests.post(ollama_url,
...
def safety_report(drugname):
    ...
    ai_timeout  = current_app.config['AI_TIMEOUT']
    api_timeout = current_app.config['API_TIMEOUT']
    pubmed_max  = current_app.config['PUBMED_MAX_RESULTS']
    ...
    try:
        search_res = http_requests.get(
            ...
            params={'db': 'pubmed', 'term': f'{drugname} adverse event safety pharmacovigilance',
                    'retmax': pubmed_max, 'retmode': 'json', 'sort': 'relevance'},
            timeout=api_timeout)
...
## 6. 결론"""
    ...
    return jsonify({
        'drug': drugname,
        'stats': {'total_reports': total, 'age_avg': age_avg,
                  'death_cnt': death_cnt, 'hosp_cnt': hosp_cnt, 'top_reactions': top_reac},
...
def safety_report_pdf(drugname):
    ...
    try:
        search_res = http_requests.get(
            'https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi',
            params={'db': 'pubmed', 'term': f'{drugname} adverse event safety',
                    'retmax': pubmed_max, 'retmode': 'json', 'sort': 'relevance'},
            timeout=api_timeout)
        ids = search_res.json()['esearchresult']['idlist']
        if ids:
...
6. 결론"""
    ...
    title_style  = ParagraphStyle('title',  fontSize=16, spaceAfter=10,
                                  textColor=colors.HexColor('#10b981'), fontName=korean_font)
    sub_style    = ParagraphStyle('sub',    fontSize=10, spaceAfter=6,
                                  textColor=colors.HexColor('#374151'), fontName=korean_font)
    header_style = ParagraphStyle('header', fontSize=12, spaceAfter=6,
                                  textColor=colors.HexColor('#10b981'), fontName=korean_font)
    body_style   = ParagraphStyle('body',   fontSize=10, spaceAfter=6,
                                  textColor=colors.HexColor('#374151'), fontName=korean_font, leading=16)

    story = []
    story.append(Paragraph("AI 약물 안전성 종합 리포트", title_style))
    story.append(Paragraph(f"Drug: {drugname}", sub_style))
