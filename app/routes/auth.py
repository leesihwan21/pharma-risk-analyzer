    ...
    if request.method == "POST":
        ...
        if user and user.locked_until and user.locked_until > datetime.utcnow():
            ...
            return (
                jsonify(
                    {
                        "error": f"로그인 시도 횟수 초과로 계정이 잠겼습니다. {remaining}분 후 다시 시도해주세요."
                    }
                ),
                403,
            )
        ...
        if not user or not check_password_hash(user.password_hash, password):
            if user:
                ...
                if user.login_failed_count >= 5:
                    ...
                    return (
                        jsonify(
                            {"error": "로그인 5회 실패로 계정이 30분간 잠겼습니다."}
                        ),
                        403,
                    )
                db.session.commit()
                ...
                return (
                    jsonify(
                        {
                            "error": f"아이디 또는 비밀번호가 틀렸습니다. (남은 시도: {remaining}회)"
                        }
                    ),
                    401,
                )
...
def admin_logs():
    if current_user.role != "ADMIN":
        return jsonify({"error": "관리자 권한이 필요합니다."}), 403
    logs = (
        UserActivityLog.query.order_by(UserActivityLog.created_at.desc())
        .limit(100)
        .all()
    )
...
def admin_predictions():
    if current_user.role != "ADMIN":
        return jsonify({"error": "관리자 권한이 필요합니다."}), 403
    predictions = (
        PredictionLog.query.order_by(PredictionLog.predicted_at.desc()).limit(100).all()
    )
...
@login_required
def make_admin():
    # 보안: 지정된 본인 계정만 관리자 권한 획득 가능
    allowed = os.environ.get("ADMIN_USERNAME", "leesihwan21")
    if current_user.username != allowed:
...
def forgot_password():
    ...
    reset_token = PasswordResetToken(
        user_id=user.id, token=token, expires_at=datetime.utcnow() + timedelta(hours=1)
    )
...
def reset_password(token):
    ...
    if not reset_token or reset_token.expires_at < datetime.utcnow():
        return render_template(
            "reset_password.html", error="링크가 만료됐거나 유효하지 않아요"
        )
    if request.method == "GET":
